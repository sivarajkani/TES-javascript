...
$(document).ready(function() {
	...
	$("#laneNavNew, #laneNavPlus").click(function(e) {
		...
		if (!$("#saveRates").hasClass("disabled") && authorizedWorkFlowStatus_change(quotetype, approvdsts)) {
		...
		if (importMode){
			...
			for (var x = 0; x < lanes.length; x++) {
				...
				if (lanes[x].type == "Chg"){
					...
					lanes[x].workflowstatus = lanes[x].saveWorkFlowStatus;
			...
			if (!inLaneListMode && !$("#saveRates").hasClass("disabled") && !$("#saveRates").hasClass("lockDisable")
					            && authorizedWorkFlowStatus_change(quotetype, approvdsts)) {
	...
	if (laneListTable === undefined){
		laneListTable = $('#laneListTable').DataTable( {
	        ...
	        createdRow: function( row, data, dataIndex) {
				...
				// Color invalid cells
				if (importMode){
					// Hide Origin Zone, Dest Zone
					laneListTable.column([3, 5]).visible( false );
					// Show UOM and Min Charge
					...
					if (data.workflowstatusValid != "true" && data.type == "Chg") {
						var workFlowStatusColumn = 5;

						$('td', row).eq(workFlowStatusColumn).addClass("invalidField");
						...
						if (lanenumberStr == data.data.lanenumber){
							...
							switch (inputID){
								...
								case "priminchr":
									...
									break;
								case "workflowstatus":
									lane.workflowstatus = inputObject.val();
									if (lane.idpqlandtl != 0){
										lane.workflowstatus = inputObject.val();
										if (lane.workflowstatus != "" && lane.workflowstatus != null){
											$(".DTE_Field_Input").parents("td").removeClass("invalidField");
										}
									}
									break;
				...
				if ((importMode && $(this).hasClass("invalidField")) || $(this).hasClass("editCustLane")) {
					...
					if ($(this).hasClass("invalidField")){
						...
						switch (inputID){
							...
							case "workflowstatus":
								// Always rebuild the select list
								//selectField.children("option:not(:first)").remove();
								
								var thisRowLaneData = laneListTable.row( $(this).parents('tr') ).data();
								var thisLane = Number(thisRowLaneData.lanenumber)
								
								//if (selectField.children("option").length == 0){
								if (thisLane != lastSelectedWorkFlowLane){
									lastSelectedWorkFlowLane = thisLane;
									
									//selectField.children("option:not(:first)").remove();
									selectField.children("option").remove();
									 // Build select list for valid work flow status
									var thisIdx = Number(thisRowLaneData.lanenumber) - 1;
									lastSelectedWorkFlowLane = thisLane;							
									// When quote is a Spot, or current workflow is TBP, Pub, Cmp - load current workflow
									if (lanes[thisIdx].saveQuoteType == "Spot" ||
										(lanes[thisIdx].saveWorkFlowStatus == "TBP" || 
										 lanes[thisIdx].saveWorkFlowStatus == "Pub" || 
										 lanes[thisIdx].saveWorkFlowStatus == "Cmp")){
										// Find values for current work flow status
										for (var w = 0; w < workFlowStatus_values.length; w++) { 
								        	if (workFlowStatus_values[w].sdesc == lanes[thisIdx].saveWorkFlowStatus){
								        		selectField.append("<option value='" + workFlowStatus_values[w].sdesc + 
							        			//		"' sdesc='" + workFlowStatus_values[w].sdesc +
							        			//        "' desc='" + workFlowStatus_values[w].desc +
							        			  //      "' code='" + workFlowStatus_values[w].code +
							        			        "'>" + workFlowStatus_values[w].desc + "</option>");
								        		break;
								           	};
								        }
									} else {
										// Build Work Flow status select options
										for (var w = 0; w < workFlowStatus_values.length; w++) {
											// Skip Published Work Flow Status
											if (workFlowStatus_values[w].sdesc != "Pub"){
												// Quote is not priced - status cannot be Load Moved, To Be Published and Sent for Signature
												if (lanes[thisIdx].proposedrate == "" && (workFlowStatus_values[w].sdesc == "Mov" 
													|| workFlowStatus_values[w].sdesc == "TBP" || workFlowStatus_values[w].sdesc == "Sig")){
												} else {
													selectField.append("<option value='" + workFlowStatus_values[w].sdesc + 
								        				//	"' sdesc='" + workFlowStatus_values[w].sdesc +
								        			    //    "' desc='" + workFlowStatus_values[w].desc +
								...
								selectField.blur(function(){
									laneListEditor.set('workflowstatus', $(this).val()).submit();
								});
...
function createNewLane() {
 ...
 // Default Lane Type to Inquiry
	...
	$("#ctrqVolume .volumetricVal").html("");
    $("#ctrqVolume .volumetricInput").html("");
	$("#cfilVolume .volumetricVal").html("");
    $("#cfilVolume .volumetricInput").html("");
