  ...
  function SelectUOMEditor(args) {
	    ...
	    this.init = function () {
	        ...
	        for (i in opt_values) {
	            ...
	            // Skip UOM Option of "PCT" when Charge is Linehaul
	            if (args.item.linecode == "100" && v == "PCT"){
	            	continue;
	       		}
	    ...
	    this.applyValue = function (item, state) {
	        ...
	        // Set the Value of Priced Rate and Extended to 0
	     // Calculate Extended Amount based on UOM
			switch (item.propuom){
			case "F/R":
				item.propamt = Number(item.proprate).toFixed(2);
				break;
			case "MLS":
				// Drop Decimal Point to calculate, apply decimal, adjust to 2 decimals
				item.propamt = (((item.proprate * 1000) * item.rmsqty)/1000).toFixed(2);
				break;
			case "PCT":
				// PCT Amount is Calculated as a Percent of Total Linehaul which is always in Row 0
				item.propamt = (((item.proprate * 100) * (chrg[0].propamt * 100)) /10000).toFixed(2);
				break;
			default:
				item.propamt = 0;
			}
			var linehaulamount = chrg[0].propamt;
  ...
  function PriceRateEditor(args) {
	    ...
	    this.applyValue = function (item, state) {
			...
			case "PCT":
				// PCT Amount is Calculated as a Percent of Total Linehaul which is always in Row 0
				// Drop Decimal Point to calculate, apply decimal, adjust to 2 decimals
				// PCT = 2 decimals, Amount = 2 decimals
				item.propamt = (((item.proprate * 100) * (chrg[0].propamt * 100)) /10000).toFixed(2);
				break;
			default:
				item.propamt = 0;
